...
function CreateEventModalContent({ onClose, onCreated }){
 const [title,setTitle]=useState('')
 ...
 const [churchLocations,setChurchLocations]=useState([])
  const [locationType,setLocationType]=useState('CHURCH')
  const [venueName,setVenueName]=useState('')
  const [address,setAddress]=useState('')
  const [mapInfo,setMapInfo]=useState('')
  const [isOnline,setIsOnline]=useState(false)
  const [onlineUrl,setOnlineUrl]=useState('')
  const [onlineAccessInstructions,setOnlineAccessInstructions]=useState('')
 ...
 const [status,setStatus]=useState('DRAFT')
  ...
  const [organizerContactPerson,setOrganizerContactPerson]=useState('')
  const [organizerContactPhone,setOrganizerContactPhone]=useState('')
  const [busTransportEnabled,setBusTransportEnabled]=useState(false)
  const [sendRegistrationConfirmation,setSendRegistrationConfirmation]=useState(true)
 const [saving,setSaving]=useState(false)
 const [error,setError]=useState('')
 useEffect(()=>{const closeOnEscape=(event)=>{if(event.key==='Escape')onClose()};const previousOverflow=document.body.style.overflow;document.body.style.overflow='hidden';window.addEventListener('keydown',closeOnEscape);return()=>{document.body.style.overflow=previousOverflow;window.removeEventListener('keydown',closeOnEscape)}},[onClose])
 useEffect(()=>{fetchAdminApi('/api/admin/reference/church-locations').then(({churchLocations:items=[]})=>setChurchLocations(items)).catch(()=>{})},[])
 const chooseImage=(event)=>{const file=event.target.files?.[0];if(!file)return;const reader=new FileReader();reader.onload=()=>setImageUrl(String(reader.result));reader.readAsDataURL(file)}
  const submit=async(event)=>{event.preventDefault();setError('');if(endsAt&&new Date(endsAt)<=new Date(startsAt)){setError('End time must be after start time.');return}if(registrationRequired&&!registrationCapacity){setError('Registration capacity is required when registration is enabled.');return}if(isOnline&&!onlineUrl){setError('Add a streaming or event link.');return}setSaving(true);try{const {event:created}=await fetchAdminApi('/api/admin/events',{method:'POST',body:JSON.stringify({title,description,startsAt:new Date(startsAt).toISOString(),endsAt:endsAt?new Date(endsAt).toISOString():null,location:locationType==='ONLINE'?'Online':venueName,isPaid,price:isPaid?Number(price):undefined,imageUrl,notifyOnPublish,status,eventType,registrationRequired,registrationOpensAt:registrationOpensAt?new Date(registrationOpensAt).toISOString():null,registrationClosesAt:registrationClosesAt?new Date(registrationClosesAt).toISOString():null,registrationCapacity:registrationRequired?Number(registrationCapacity):null,allowWaitlist,organizerUnit,organizerContactPerson,organizerContactPhone,isOnline:locationType==='ONLINE'||isOnline,onlineUrl:onlineUrl||null,onlineAccessInstructions:onlineAccessInstructions||null,busTransportEnabled,locationType,venueName,address,mapInfo,sendRegistrationConfirmation})});onCreated(created)}catch(requestError){setError(requestError.message||'Event could not be created.')}finally{setSaving(false)}}
 return <div className="event-modal-backdrop" role="dialog" aria-modal="true" onMouseDown={(event)=>{if(event.target===event.currentTarget)onClose()}}><form className="event-modal" onSubmit={submit}><div className="event-modal-header"><div><h2>Create Event</h2><p>Add a new event to the GIC platform</p></div><button type="button" className="event-modal-close" aria-label="Close create event" onClick={onClose}><X size={18}/></button></div><div className="event-modal-body"><label className="modern-field full"><span>Title</span><input value={title} onChange={(event)=>setTitle(event.target.value)} placeholder="e.g. Sunday Service" required /></label><label className="modern-field full"><span>Description</span><textarea value={description} onChange={(event)=>setDescription(event.target.value)} placeholder="Share the key details members should know..." rows="4" /></label><label className="modern-field full"><span>Event image</span><span className="upload-zone">{imageUrl?<img src={imageUrl} alt="Event preview"/>:<><ImagePlus size={24}/><b>Upload image or drag & drop</b><small>PNG, JPG up to 5MB</small></>}<input type="file" accept="image/*" onChange={chooseImage}/></span></label><div className="modern-field-grid"><label className="modern-field"><span>Start date & time</span><input type="datetime-local" value={startsAt} onChange={(event)=>setStartsAt(event.target.value)} required /></label><label className="modern-field"><span>End date & time</span><input type="datetime-local" value={endsAt} onChange={(event)=>setEndsAt(event.target.value)} /></label></div><label className="modern-field full"><span>Event type</span><select value={eventType} onChange={(event)=>setEventType(event.target.value)}>{['Service','Conference','Meeting','Outreach','Special Event','Other'].map((item)=><option key={item}>{item}</option>)}</select></label><div className="modern-field-grid"><label className="modern-field"><span>Location type</span><select value={locationType} onChange={(event)=>{setLocationType(event.target.value);if(event.target.value==='ONLINE')setIsOnline(true)}}><option value="CHURCH">Church location</option><option value="PHYSICAL">Other physical location</option><option value="ONLINE">Online</option></select></label>{locationType==='CHURCH'?<label className="modern-field"><span>Church location</span><select value={venueName} onChange={(event)=>{const selected=churchLocations.find((item)=>item.name===event.target.value);setVenueName(selected?.name||'');setAddress(selected?.address||'')}} required><option value="">Choose a church location</option>{churchLocations.map((item)=><option key={item.id} value={item.name}>{item.name}</option>)}</select></label>:locationType!=='ONLINE'&&<label className="modern-field"><span>Venue name</span><input value={venueName} onChange={(event)=>setVenueName(event.target.value)} placeholder="Venue name" required/> </label>}</div>{locationType!=='ONLINE'&&<div className="modern-field-grid"><label className="modern-field"><span>Address</span><input value={address} onChange={(event)=>setAddress(event.target.value)} placeholder="Street address"/></label><label className="modern-field"><span>Map information (optional)</span><input value={mapInfo} onChange={(event)=>setMapInfo(event.target.value)} placeholder="Map link or directions"/></label></div>}{locationType==='CHURCH'&&venueName&&<p className="muted">Use the seeded church address above; service times: {churchLocations.find((item)=>item.name===venueName)?.serviceTimes||'Not published'}.</p>}{(locationType==='ONLINE'||isOnline)&&<div className="modern-field-grid"><label className="modern-field"><span>Streaming / event link</span><input type="url" value={onlineUrl} onChange={(event)=>setOnlineUrl(event.target.value)} placeholder="https://..." required /></label><label className="modern-field"><span>Access instructions</span><input value={onlineAccessInstructions} onChange={(event)=>setOnlineAccessInstructions(event.target.value)} placeholder="How members should join"/></label></div>}<div className="modern-section-heading">Registration</div><label className="modern-toggle"><span><b>Registration required</b><small>Members reserve a place for this event</small></span><input type="checkbox" checked={registrationRequired} onChange={(event)=>setRegistrationRequired(event.target.checked)}/><i/></label>{registrationRequired&&<><div className="modern-field-grid"><label className="modern-field"><span>Registration opens</span><input type="datetime-local" value={registrationOpensAt} onChange={(event)=>setRegistrationOpensAt(event.target.value)}/></label><label className="modern-field"><span>Registration closes</span><input type="datetime-local" value={registrationClosesAt} onChange={(event)=>setRegistrationClosesAt(event.target.value)}/></label></div><div className="modern-field-grid"><label className="modern-field"><span>Maximum capacity</span><input type="number" min="1" value={registrationCapacity} onChange={(event)=>setRegistrationCapacity(event.target.value)} required/></label><label className="modern-toggle"><span><b>Allow waitlist</b><small>Accept registrations after capacity</small></span><input type="checkbox" checked={allowWaitlist} onChange={(event)=>setAllowWaitlist(event.target.checked)}/><i/></label></div></>}<div className="modern-section-heading">Organizer</div><div className="modern-field-grid"><label className="modern-field"><span>Organizing unit / ministry</span><input value={organizerUnit} onChange={(event)=>setOrganizerUnit(event.target.value)}/></label><label className="modern-field"><span>Contact person</span><input value={organizerContactPerson} onChange={(event)=>setOrganizerContactPerson(event.target.value)}/></label></div><label className="modern-field full"><span>Contact phone</span><input value={organizerContactPhone} onChange={(event)=>setOrganizerContactPhone(event.target.value)}/></label><div className="modern-section-heading">Options</div><div className="modern-toggle-row"><label className="modern-toggle"><span><b>Paid event</b><small>Require payment to register</small></span><input type="checkbox" checked={isPaid} onChange={(event)=>setIsPaid(event.target.checked)}/><i/></label><label className="modern-toggle"><span><b>Send notification on publish</b><small>Notify members when live</small></span><input type="checkbox" checked={notifyOnPublish} onChange={(event)=>setNotifyOnPublish(event.target.checked)}/><i/></label><label className="modern-toggle"><span><b>Online event</b><small>Show link and access instructions</small></span><input type="checkbox" checked={isOnline} onChange={(event)=>setIsOnline(event.target.checked)}/><i/></label><label className="modern-toggle"><span><b>Bus transportation</b><small>Add pickup locations after creating the event</small></span><input type="checkbox" checked={busTransportEnabled} onChange={(event)=>setBusTransportEnabled(event.target.checked)}/><i/></label><label className="modern-toggle"><span><b>Send registration confirmation</b><small>Push a confirmation immediately after signup</small></span><input type="checkbox" checked={sendRegistrationConfirmation} onChange={(event)=>setSendRegistrationConfirmation(event.target.checked)}/><i/></label></div>{isPaid&&<label className="modern-field full"><span>Ticket price</span><input type="number" min="0" value={price} onChange={(event)=>setPrice(event.target.value)} placeholder="0" required /></label>}<label className="modern-field full"><span>Status</span><select value={status} onChange={(event)=>setStatus(event.target.value)}><option value="DRAFT">Draft</option><option value="PUBLISHED">Published</option><option value="CANCELLED">Cancelled</option></select></label>{error&&<div className="event-form-error">{error}</div>}</div><div className="event-modal-footer"><button type="submit" className="btn primary wide" disabled={saving}>{saving?'Creating event...':'Create Event'}</button></div></form></div>
  return <div className="event-modal-backdrop" role="dialog" aria-modal="true" onMouseDown={(event)=>{if(event.target===event.currentTarget)onClose()}}><form className="event-modal" onSubmit={submit}><div className="event-modal-header"><div><h2>Create Event</h2><p>Add a new event to the GIC platform</p></div><button type="button" className="event-modal-close" aria-label="Close create event" onClick={onClose}><X size={18}/></button></div><div className="event-modal-body"><label className="modern-field full"><span>Title</span><input value={title} onChange={(event)=>setTitle(event.target.value)} placeholder="e.g. Sunday Service" required /></label><label className="modern-field full"><span>Description</span><textarea value={description} onChange={(event)=>setDescription(event.target.value)} placeholder="Share the key details members should know..." rows="4" /></label><label className="modern-field full"><span>Event image</span><span className="upload-zone">{imageUrl?<img src={imageUrl} alt="Event preview"/>:<><ImagePlus size={24}/><b>Upload image or drag & drop</b><small>PNG, JPG up to 5MB</small></>}<input type="file" accept="image/*" onChange={chooseImage}/></span></label><div className="modern-field-grid"><label className="modern-field"><span>Start date & time</span><input type="datetime-local" value={startsAt} onChange={(event)=>setStartsAt(event.target.value)} required /></label><label className="modern-field"><span>End date & time</span><input type="datetime-local" value={endsAt} onChange={(event)=>setEndsAt(event.target.value)} /></label></div><label className="modern-field full"><span>Event type</span><select value={eventType} onChange={(event)=>setEventType(event.target.value)}>{['Service','Conference','Meeting','Outreach','Special Event','Other'].map((item)=><option key={item}>{item}</option>)}</select></label><div className="modern-field-grid"><label className="modern-field"><span>Location type</span><select value={locationType} onChange={(event)=>{setLocationType(event.target.value);if(event.target.value==='ONLINE')setIsOnline(true)}}><option value="CHURCH">Church location</option><option value="PHYSICAL">Other physical location</option><option value="ONLINE">Online</option></select></label>{locationType!=='ONLINE'&&<label className="modern-field"><span>Venue name</span><input value={venueName} onChange={(event)=>setVenueName(event.target.value)} placeholder={locationType==='CHURCH'?'Main Auditorium':'Venue name'}/></label>}</div>{locationType!=='ONLINE'&&<div className="modern-field-grid"><label className="modern-field"><span>Address</span><input value={address} onChange={(event)=>setAddress(event.target.value)} placeholder="Street address"/></label><label className="modern-field"><span>Map information (optional)</span><input value={mapInfo} onChange={(event)=>setMapInfo(event.target.value)} placeholder="Map link or directions"/></label></div>}{(locationType==='ONLINE'||isOnline)&&<div className="modern-field-grid"><label className="modern-field"><span>Streaming / event link</span><input type="url" value={onlineUrl} onChange={(event)=>setOnlineUrl(event.target.value)} placeholder="https://..." required /></label><label className="modern-field"><span>Access instructions</span><input value={onlineAccessInstructions} onChange={(event)=>setOnlineAccessInstructions(event.target.value)} placeholder="How members should join"/></label></div>}<div className="modern-section-heading">Registration</div><label className="modern-toggle"><span><b>Registration required</b><small>Members reserve a place for this event</small></span><input type="checkbox" checked={registrationRequired} onChange={(event)=>setRegistrationRequired(event.target.checked)}/><i/></label>{registrationRequired&&<><div className="modern-field-grid"><label className="modern-field"><span>Registration opens</span><input type="datetime-local" value={registrationOpensAt} onChange={(event)=>setRegistrationOpensAt(event.target.value)}/></label><label className="modern-field"><span>Registration closes</span><input type="datetime-local" value={registrationClosesAt} onChange={(event)=>setRegistrationClosesAt(event.target.value)}/></label></div><div className="modern-field-grid"><label className="modern-field"><span>Maximum capacity</span><input type="number" min="1" value={registrationCapacity} onChange={(event)=>setRegistrationCapacity(event.target.value)} required/></label><label className="modern-toggle"><span><b>Allow waitlist</b><small>Accept registrations after capacity</small></span><input type="checkbox" checked={allowWaitlist} onChange={(event)=>setAllowWaitlist(event.target.checked)}/><i/></label></div></>}<div className="modern-section-heading">Organizer</div><div className="modern-field-grid"><label className="modern-field"><span>Organizing unit / ministry</span><input value={organizerUnit} onChange={(event)=>setOrganizerUnit(event.target.value)}/></label><label className="modern-field"><span>Contact person</span><input value={organizerContactPerson} onChange={(event)=>setOrganizerContactPerson(event.target.value)}/></label></div><label className="modern-field full"><span>Contact phone</span><input value={organizerContactPhone} onChange={(event)=>setOrganizerContactPhone(event.target.value)}/></label><div className="modern-section-heading">Options</div><div className="modern-toggle-row"><label className="modern-toggle"><span><b>Paid event</b><small>Require payment to register</small></span><input type="checkbox" checked={isPaid} onChange={(event)=>setIsPaid(event.target.checked)}/><i/></label><label className="modern-toggle"><span><b>Send notification on publish</b><small>Notify members when live</small></span><input type="checkbox" checked={notifyOnPublish} onChange={(event)=>setNotifyOnPublish(event.target.checked)}/><i/></label><label className="modern-toggle"><span><b>Online event</b><small>Show link and access instructions</small></span><input type="checkbox" checked={isOnline} onChange={(event)=>setIsOnline(event.target.checked)}/><i/></label><label className="modern-toggle"><span><b>Bus transportation</b><small>Add pickup locations after creating the event</small></span><input type="checkbox" checked={busTransportEnabled} onChange={(event)=>setBusTransportEnabled(event.target.checked)}/><i/></label><label className="modern-toggle"><span><b>Send registration confirmation</b><small>Push a confirmation immediately after signup</small></span><input type="checkbox" checked={sendRegistrationConfirmation} onChange={(event)=>setSendRegistrationConfirmation(event.target.checked)}/><i/></label></div>{isPaid&&<label className="modern-field full"><span>Ticket price</span><input type="number" min="0" value={price} onChange={(event)=>setPrice(event.target.value)} placeholder="0" required /></label>}<label className="modern-field full"><span>Status</span><select value={status} onChange={(event)=>setStatus(event.target.value)}><option value="DRAFT">Draft</option><option value="PUBLISHED">Published</option><option value="CANCELLED">Cancelled</option></select></label>{error&&<div className="event-form-error">{error}</div>}</div><div className="event-modal-footer"><button type="submit" className="btn primary wide" disabled={saving}>{saving?'Creating event...':'Create Event'}</button></div></form></div>
}

function CreateEventModal(props){
 return createPortal(<CreateEventModalContent {...props} />, document.body)
}
...
function AdminEventRegistrations({eventId}){
 const [data,setData]=useState({registrations:[],passengerCounts:[]})
 const [loading,setLoading]=useState(true)
 const [error,setError]=useState('')
...
function EventReminders({eventId,eventStartsAt}){
 const options=[{label:'1 day before',minutes:1440},{label:'1 hour before',minutes:60},{label:'30 minutes before',minutes:30}]
 const [selected,setSelected]=useState([])
 const [custom,setCustom]=useState('')
 const [saving,setSaving]=useState(false)
 const [message,setMessage]=useState('')
 useEffect(()=>{fetchAdminApi(`/api/admin/events/${eventId}/reminders`).then(({reminders=[]})=>{setSelected(reminders.filter((item)=>![1440,60,30].includes(item.offsetMinutes)).map((item)=>item.offsetMinutes));setSelected((current)=>[...current,...reminders.filter((item)=>[1440,60,30].includes(item.offsetMinutes)).map((item)=>item.offsetMinutes)])}).catch(()=>{})},[eventId])
 const save=async()=>{setSaving(true);setMessage('');const customMinutes=custom?Number(custom):0;const offsets=[...selected,...(customMinutes>0?[customMinutes]:[])];try{await fetchAdminApi(`/api/admin/events/${eventId}/reminders`,{method:'PUT',body:JSON.stringify({offsets})});setMessage('Reminder settings saved. They will use the existing FCM push pipeline.')}catch(error){setMessage(error.message||'Could not save reminders.')}finally{setSaving(false)}}
 return <div className="event-admin-section"><h3>Event reminders</h3><p className="muted">Send push reminders to confirmed registrants. No email, SMS, or WhatsApp is used.</p><div className="reminder-options">{options.map((option)=><label className="check" key={option.minutes}><input type="checkbox" checked={selected.includes(option.minutes)} onChange={()=>setSelected((current)=>current.includes(option.minutes)?current.filter((value)=>value!==option.minutes):[...current,option.minutes])}/>{option.label}</label>)}</div><label className="modern-field"><span>Custom minutes before start</span><input type="number" min="1" value={custom} onChange={(event)=>setCustom(event.target.value)} placeholder="e.g. 180"/></label><button className="btn primary" onClick={save} disabled={saving}>{saving?'Saving...':'Save reminders'}</button>{message&&<p className="saved">{message}</p>}<small className="muted">Event starts {new Date(eventStartsAt).toLocaleString()}.</small></div>
}
...
function AdminEventDetails(){
 ...
 return <Page title={event.title} subtitle={`${event.eventType} · ${event.status}`}><div className="detail-toolbar modern-detail-toolbar"><Link to="/events"><ArrowLeft size={16}/> Back to Events</Link><span className="badge success">{event.registrationCount||0} registrations</span></div><Card className="modern-event-detail"><div className="modern-event-summary"><div><span className={'badge '+(event.status==='PUBLISHED'?'success':'draft')}>{event.status}</span><h2>{event.title}</h2><p><CalendarDays size={14}/> {new Date(event.startsAt).toLocaleString()} {event.endsAt&&`– ${new Date(event.endsAt).toLocaleString()}`}</p><p><MapPin size={14}/> {event.locationType==='ONLINE'?'Online':event.venueName||event.location||'Location to be announced'}{event.address&&` · ${event.address}`}</p></div><div className="event-detail-facts"><b>{event.registrationRequired?'Registration required':'Registration not required'}</b>{event.registrationCapacity&&<span>Capacity {event.registrationCapacity}</span>}{event.organizerUnit&&<span>Organizer: {event.organizerUnit}</span>}{event.isOnline&&<span>Online link configured</span>}{event.busTransportEnabled&&<span>Bus transportation enabled</span>}</div></div><div className="tabs big">{tabs.map((item)=><button key={item} className={tab===item?'active':''} onClick={()=>setTab(item)}>{item}</button>)}</div>{tab==='Overview'&&<div className="event-admin-section"><div className="modern-profile-grid"><div><small>Description</small><b>{event.description||'—'}</b></div><div><small>Contact person</small><b>{event.organizerContactPerson||'—'} {event.organizerContactPhone&&`· ${event.organizerContactPhone}`}</b></div><div><small>Online access</small><b>{event.onlineUrl||'—'}</b></div><div><small>Confirmation push</small><b>{event.sendRegistrationConfirmation?'Enabled':'Disabled'}</b></div></div></div>}{tab==='Registrations'&&<AdminEventRegistrations eventId={id}/>} {tab==='Bus pickups'&&<PickupLocations eventId={id}/>} {tab==='Reminders'&&<EventReminders eventId={id} eventStartsAt={event.startsAt}/>}</Card></Page>
}
