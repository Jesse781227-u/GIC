...
function PickupLocations({eventId}){
 const blank={locationName:'',addressLandmark:'',pickupTime:'',capacity:'',notes:'',active:true}
 const [items,setItems]=useState([])
 ...
 const edit=(item)=>setEditing(item.id)||setForm({locationName:item.locationName,addressLandmark:item.addressLandmark,pickupTime:new Date(item.pickupTime).toISOString().slice(0,16),capacity:String(item.capacity),notes:item.notes||'',active:item.active})
